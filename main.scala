...
def run() = {
    ...
    def dScore(f: Vector[String], x: Int) = {
        def dwinner(f: Vector[String], x: Int): Unit = {
    ...
    //kallar på dScore, första argument(f) = dbet"namn", andra argument(x) = poängvariablen ("namn")
    dScore(f = dBetAnton, x = Anton)
    println(Anton)
}